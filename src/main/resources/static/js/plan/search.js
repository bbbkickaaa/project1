...
//                                            body.appendChild(script2);
		...
		document.getElementById('searchForm').addEventListener('submit', function (event) {
            event.preventDefault();  // 폼의 기본 동작 방지 (페이지 새로고침 등)
            ...
            fetch(`/search?area=${area}&keyword=${keyword}&startDate=${startDateString}&endDate=${endDateString}&date=${date}&title=${title}`, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',  // 필요에 따라 헤더를 설정
                    // 다른 헤더들도 필요에 따라 추가 가능
                },
